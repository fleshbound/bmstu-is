...
static void sub_bytes(byte *a,int n) {
    int i;
    for (i=0; i<n; i++) {
        a[i] = lookup_sbox[(int)a[i]];
    }
}
static void sub_bytes_inv(byte *a,int n) {
    int i;
    for (i=0; i<n; i++) {
        a[i] = lookup_sbox_inv[(int)a[i]];
    }
}
...
static void shift_rows(byte *state) {
    ...
    for (i=0; i<16; i++) {
        state[i]=temp[(int)shift_rows_table[i]];
    }
}
static void shift_rows_inv(byte *state) {
    ...
    memcpy(temp,state,16);
    for (i=0; i<16; i++) {
        state[i]=temp[(int)shift_rows_table_inv[i]];
    }
}
...
static void mix_col (byte *state) {
    ...
    byte a2 = state[2];
    byte a3 = state[3];
    state[0] = lookup_g2[(int)a0] ^ lookup_g3[(int)a1] ^ a2 ^ a3;
    state[1] = lookup_g2[(int)a1] ^ lookup_g3[(int)a2] ^ a3 ^ a0;
    state[2] = lookup_g2[(int)a2] ^ lookup_g3[(int)a3] ^ a0 ^ a1;
    state[3] = lookup_g2[(int)a3] ^ lookup_g3[(int)a0] ^ a1 ^ a2;
}
...
static void mix_col_inv (byte *state) {
    ...
    byte a2 = state[2];
    byte a3 = state[3];
    state[0] = lookup_g14[(int)a0] ^ lookup_g9[(int)a3] ^ lookup_g13[(int)a2] ^ lookup_g11[(int)a1];
    state[1] = lookup_g14[(int)a1] ^ lookup_g9[(int)a0] ^ lookup_g13[(int)a3] ^ lookup_g11[(int)a2];
    state[2] = lookup_g14[(int)a2] ^ lookup_g9[(int)a1] ^ lookup_g13[(int)a0] ^ lookup_g11[(int)a3];
    state[3] = lookup_g14[(int)a3] ^ lookup_g9[(int)a2] ^ lookup_g13[(int)a1] ^ lookup_g11[(int)a0];
}
...
void EncryptAES256(const byte *msg, const byte *key, byte *c) {
    int i;
...
void DecryptAES256(const byte *c, const byte *key, byte *m) {
    int i;
